...
el("diagnosticVersion").textContent=`${manifest.version} · pannello strutturato`;
...
el("copyDiagnostics").onclick=async()=>{const payload=`GPhoto2MyCloud ${manifest.version}\nID: ${chrome.runtime.id}\nHost: ${el("hostState").textContent}\nUI: structured-progress-v2`;await navigator.clipboard.writeText(payload);addLog("success","Diagnostica copiata",payload.replaceAll("\n"," · "));};
...
addLog("info","Interfaccia aggiornata caricata",`versione ${manifest.version} · build structured-progress-v2`);
