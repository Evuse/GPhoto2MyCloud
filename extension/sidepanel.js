...
el("diagnosticVersion").textContent=`${manifest.version} · FINAL-PROD-3.4-AUTO-RECOVERY`;
...
el("start").onclick=async()=>{try{const cfg=settings();await chrome.storage.local.set({settings:cfg,syncDesired:true});const host=await chrome.runtime.sendMessage({type:"hostStatus",destination:cfg.destination});if(!host.ok)throw new Error(host.error);const tab=await photosTab();await chrome.storage.session.set({automationTabId:tab.id});setRunning(true);setProgress("process",5);addLog("info","Backup persistente avviato",`scheda ${tab.id}, lotti iniziali da ${cfg.batchSize}`);await chrome.tabs.sendMessage(tab.id,{type:"start",settings:cfg});}catch(error){el("dot").className="error";el("message").textContent=error.message;el("runBadge").className="badge error";el("runBadge").textContent="ERRORE";addLog("error","Avvio fallito",error.message);}};
...
addLog("success","Build corretta caricata",`versione ${manifest.version} · FINAL-PROD-3.4-AUTO-RECOVERY`);
