"use strict";
const ids = ["destination", "extractedFolder", "downloadRoot", "batchSize", "clickDelayMs", "settleSeconds", "retryDelaySeconds", "downloadTimeoutMinutes", "verifyCopies", "keepArchives"];
...
el("diagnosticVersion").textContent=`${manifest.version} · FINAL-PROD-3.6-INSTANT-ANCHOR`;
...
el("start").onclick=async()=>{try{const cfg=settings();await chrome.storage.local.set({settings:cfg,syncDesired:true});const host=await chrome.runtime.sendMessage({type:"hostStatus",destination:cfg.destination});if(!host.ok)throw new Error(host.error);const tab=await photosTab();await chrome.storage.session.set({automationTabId:tab.id});setRunning(true);setProgress("process",5);addLog("info","Verifica collegamento alla pagina",`scheda ${tab.id}`);const content=await chrome.runtime.sendMessage({type:"ensureAutomationContent",tabId:tab.id});if(!content?.ok)throw new Error(content?.error||"Impossibile installare il content script");addLog("success","Automazione collegata",`content script ${content.version||manifest.version}`);await chrome.tabs.sendMessage(tab.id,{type:"start",settings:cfg});}catch(error){setRunning(false);el("dot").className="error";el("message").textContent=error.message;el("runBadge").className="badge error";el("runBadge").textContent="ERRORE";addLog("error","Collegamento non riuscito; ripresa automatica attiva",error.message);}};
...
el("reset").onclick=async()=>{if(!confirm("Azzerare il registro? Tutte le foto saranno considerate nuovamente."))return;await chrome.storage.local.remove(["completedPhotoIds","resumeAnchor","nasResumePhotoId"]);addLog("success","Registro elementi e posizione di ripresa azzerati");};
...
addLog("success","Build corretta caricata",`versione ${manifest.version} · FINAL-PROD-3.6-INSTANT-ANCHOR`);
